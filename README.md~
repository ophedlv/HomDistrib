...
## ABOUT
Tara Oceans expeditions have produced a huge amount of genomics and transcriptomics data - metaomics in short - which provide a global and unprecedented view of planktonic diversity. In particular, a global atlas of eucaryotic genes has been produced (Carradec et al., submitted) and metagenomics and metatranscriptomics data are available. From eukaryotic TARA samples (68 stations, 2 depths, 4 size classes), after high-coverage polyA-based RNA-Seq and cDNA assembly, unigenes have been defined (longest sequences of each cluster of contigs - cutoff identity > 95% and length > 150 pb). Among these data, specific homologs of sequences of interest could be searched in order to explore their distribution.
...
Text outputs are always saved in two versions : a detailled one with all alignment characteristics and a synthetic one which only contains sequences identifiers.
...
- Continents : World contours shapefile used to plot results.

All needed datasets are given in the data folder. This pipeline is specificly implemented to use them. If other matrix are used, they need to be formatted in the same way.
